...
async def compute(product_id: str, margin_percent: float = 3.0):
    ...
    try:
        ...
        try:
            ...
            blockchain_successful = submit_result.get('status') == 'confirmed' if isinstance(submit_result, dict) else False
            
            # If blockchain transaction is successful, update the product's last_known_price
            if blockchain_successful:
                logger.info(f"Blockchain transaction confirmed for {product_id}, updating last_known_price to {lowest}")
        ...
        try:
            ...
            if blockchain_successful:
                logger.info(f"Price successfully updated and persisted for product {product_id}: {display} paise (blockchain: {blockchain_tx_id})")
        ...
        result = {
            ...
            'all_sources': (aggregated or {}).get('all', []),
            'blockchain_tx_id': blockchain_tx_id,
            'blockchain_successful': blockchain_successful,
            'price_updated': blockchain_successful,  # For frontend to know when to refresh
        }
        return result
    finally:
        db.close()
