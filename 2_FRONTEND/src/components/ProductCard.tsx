...
import { ShoppingBag, Heart, ChevronLeft, ChevronRight, RefreshCw, Shield } from "lucide-react";
...
export default function ProductCard({
  ...
  const effectivePrice = typeof livePrice === 'number' ? livePrice : price
        ...
        <div className="aspect-[3/4] overflow-hidden rounded-lg bg-gray-50">
          <div className={cn(
            ...
            <div className="absolute top-2 left-2 flex flex-col gap-2">
              ...
              {isBlockchainPrice && (
                <span className="inline-flex items-center gap-1 rounded-full bg-green-600 px-2 py-1 text-xs font-medium text-white animate-fade-in-fast" title={`Blockchain verified price${blockchainTxId ? ` (TX: ${blockchainTxId.slice(0, 8)}...)` : ''}`}>
        ...
        <div className="mt-4 flex flex-col">
          ...
          <div className="mt-1 flex items-center justify-between">
            ...
            {confidence && confidence > 0.8 && (
              <div className="flex items-center gap-1" title={`Price confidence: ${Math.round(confidence * 100)}%`}>
                <div className={cn(
                  "h-2 w-2 rounded-full",
                  confidence > 0.9 ? "bg-green-500" : "bg-yellow-500"
                )} />
              </div>
            )}
          </div>
